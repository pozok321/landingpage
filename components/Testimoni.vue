<script setup>
    ...
    const api = useApi()
    const getTestimonialsData = ref({ testimonials: [] })

    const getTestimonials = async () => {
        let currentLang = 'id'
        if (process.client) {
            currentLang = localStorage.getItem('user-locale') || 'id'
        }
        api({
            url: `/v1/frontpage/gettestimonials?lang=${currentLang}`, 
            method: 'GET'
        }).then((res) => {
            getTestimonialsData.value = res.data
            if (res.data.testimonials && res.data.testimonials.length > 0) {
                const langCode = res.data.testimonials[0].language_code
                if (process.client) {
    ...
    <section class="py-24 overflow-hidden">
        <div class="max-w-7xl mx-auto container">
            ...
            <div class="flex overflow-x-auto gap-8 pb-10 hide-scrollbar snap-x snap-mandatory">
                <!-- Looping dari getTestimonialsData.testimonials -->
                <div v-for="(testi, index) in getTestimonialsData.testimonials" :key="testi.testimonial_id || index"
                    class="flex-shrink-0 w-[85%] md:w-[calc(40%-1rem)] snap-center">
                    
                    <!-- Box Testimoni -->
                    <div class="bg-[#edeff3] h-full p-10 rounded-[40px] flex flex-col justify-between shadow-sm hover:shadow-md transition-shadow">
                        <div>
                        ...
                        <div class="flex justify-between items-end">
                            <div class="flex items-center gap-4">
                                <!-- Tambahkan foto jika tersedia di API -->
                                <img v-if="testi.client_photo" :src="testi.client_photo" :alt="testi.client_name" 
                            ...
                            <div class="flex text-yellow-500 gap-1 mt-4">
                                <i v-for="n in testi.rating" :key="'full-' + index + n" class="bx bxs-star text-lg"></i>
                                <i v-for="n in (5 - testi.rating)" :key="'empty-' + index + n" class="bx bxs-star text-lg text-gray-300"></i>
                            </div>
                        </div>
                    </div>
                </div>
